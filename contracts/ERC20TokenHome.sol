...
contract ERC20TokenHome is ITeleporterReceiver, Ownable, ReentrancyGuard {
    ...
    mapping(address => aipPlan) public useraips;
    ...
    address[] public activeaipUsers;

    // Gelato Automation
    address public gelatoExecutor; // trusted Gelato task caller (optional)
    bool public gelatoRestricted = true; // if true, only gelatoExecutor can call exec
    ...
    event aipCreated(address indexed user, uint256 monthlyAmount, uint256 totalTarget, string goal);
    event aipDeposit(address indexed user, uint256 amount);
    ...
    event GelatoExecutorUpdated(address indexed executor, bool restricted);
    ...
    // Configure Gelato executor and restriction behavior
    function setGelatoExecutor(address executor, bool restricted) external onlyOwner {
        gelatoExecutor = executor;
        gelatoRestricted = restricted;
        emit GelatoExecutorUpdated(executor, restricted);
    }
    ...
    function createaip(
        ...
        if (!isUserInArray(msg.sender)) {
            activeaipUsers.push(msg.sender);
        }
        
        emit aipCreated(msg.sender, monthlyAmount, totalTarget, goal);
    }
    
    // === MANUAL DEPOSIT ===
    function makeaipDeposit() external nonReentrant {
        _executeaipDeposit(msg.sender);
    }
    
    // === ELIZA CONTROL FUNCTIONS ===
    function setUserAutomation(address user, bool enabled) external onlyAuthorizedAgent {
        useraips[user].automationEnabled = enabled;
    ...
    function adjustUserAmount(address user, uint256 newAmount) external onlyAuthorizedAgent {
        ...
        useraips[user].monthlyAmount = newAmount;
    ...
    function changeUserInterval(address user, uint256 newIntervalDays) external onlyAuthorizedAgent {
        ...
        useraips[user].interval = newIntervalDays * 1 days;
        emit AgentActionExecuted(msg.sender, user, "interval_changed");
    }

    // === GELATO AUTOMATION (Resolver pattern) ===
    // checker() is used by a Gelato resolver off-chain to build exec payload
    function checker()
        external
        view
        returns (bool canExec, bytes memory execPayload)
    {
        ...
        address[] memory usersToProcess = new address[](activeaipUsers.length);
        uint256 count = 0;

        for (uint256 i = 0; i < activeaipUsers.length; i++) {
            address user = activeaipUsers[i];
            aipPlan memory plan = useraips[user];
        ...
        if (count == 0) {
            return (false, bytes("no-users-due"));
        }

        address[] memory finalUsers = new address[](count);
        for (uint256 j = 0; j < count; j++) {
            finalUsers[j] = usersToProcess[j];
        }

        // Build calldata for exec
        execPayload = abi.encodeWithSelector(this.exec.selector, finalUsers);
        return (true, execPayload);
    }

    modifier onlyGelatoIfRestricted() {
        if (gelatoRestricted) {
            require(msg.sender == gelatoExecutor, "Only Gelato");
        }
        _;
    }

    // Executed by Gelato task when checker() returns true
    function exec(address[] calldata usersToProcess) external onlyGelatoIfRestricted {
        for (uint256 i = 0; i < usersToProcess.length; i++) {
            _executeaipDeposit(usersToProcess[i]);
            emit AutoDepositExecuted(usersToProcess[i], useraips[usersToProcess[i]].monthlyAmount);
        }
    }
    
    // === INTERNAL FUNCTIONS ===
    function _executeaipDeposit(address user) internal {
        aipPlan storage plan = useraips[user];
        require(plan.isActive, "No active aip");
        ...
        _distributeaipFunds(user, depositAmount);
        
        emit aipDeposit(user, depositAmount);
    }
    
    function _distributeaipFunds(address user, uint256 amount) internal {
    ...
    // === VIEW FUNCTIONS ===
    function getaipProgress(address user) external view returns (
        uint256 monthlyAmount,
        uint256 totalTarget,
        uint256 totalDeposited,
    ...
    function isUserInArray(address user) internal view returns (bool) {
        for (uint256 i = 0; i < activeaipUsers.length; i++) {
            if (activeaipUsers[i] == user) return true;
        }
        return false;
    }
    
    function getActiveaipCount() external view returns (uint256) {
        return activeaipUsers.length;
    }
}