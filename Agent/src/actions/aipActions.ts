...
// === CREATE aip ACTION ===
export const createaipAction: Action = {
  name: 'CREATE_aip',
  similes: ['SETUP_aip', 'START_INVESTMENT', 'CREATE_PLAN', 'BEGIN_aip', 'MAKE_aip', 'aip_PLAN'],
  description: 'Creates a new aip plan with real blockchain execution for AI agent',

  validate: async (runtime: IAgentRuntime, message: Memory, state: State): Promise<boolean> => {
    const text = message.content?.text?.toLowerCase() || '';
    
    const aipKeywords = ['create', 'start', 'begin', 'setup', 'make'];
    const planKeywords = ['aip', 'plan', 'investment', 'monthly', 'systematic'];
    
    const hasaipKeyword = aipKeywords.some(keyword => text.includes(keyword));
    const hasPlanKeyword = planKeywords.some(keyword => text.includes(keyword));
    
    const result = hasaipKeyword && hasPlanKeyword;
    
    if (result) {
      console.log('🎯 CREATE_aip VALIDATED AND WILL EXECUTE FOR AI AGENT');
    }
  ...
  handler: async (
    ...
    console.log('🚀🚀🚀 CREATE_aip HANDLER EXECUTING FOR AI AGENT! 🚀🚀🚀');
    ...
    try {
      ...
      console.log('✅ Web3Service found, creating aip for AI agent...');
      ...
      const aiAgentAddress = process.env.AI_AGENT_ADDRESS || '0x46d0ABa00628CeE1491f5A47c169929ff82A5b4D';
      
      console.log(`🔄 Creating aip for AI Agent: ${monthlyAmount} monthly, target: ${totalTarget}, goal: ${goal}`);
      
      const receipt = await web3Service.createaip(aiAgentAddress, monthlyAmount, totalTarget, goal);
      console.log(`✅ BLOCKCHAIN SUCCESS! Hash: ${receipt.hash}`);

      const responseContent: Content = {
        text: `🎉 REAL BLOCKCHAIN TRANSACTION EXECUTED FOR AI AGENT!

**✅ aip CREATED ON AVALANCHE FUJI**
- AI Agent: ${aiAgentAddress}
- TX Hash: ${receipt.hash}
- Block: ${receipt.blockNumber}

**💰 IMMEDIATE FIRST DEPOSIT EXECUTING**
- Amount: ${monthlyAmount} aip tokens
- Distribution: 60% Echo, 30% Fuji, 10% Dispatch
- Status: ✅ Live Transaction

**🤖 AI AUTONOMOUS MANAGEMENT ACTIVE**
- Gelato Automation: ✅ Starting in 60 seconds
...
Check transaction: https://testnet.snowtrace.io/tx/${receipt.hash}`,
      ...
      console.error('❌ CREATE_aip HANDLER ERROR:', error);
      
      const errorContent: Content = {
        text: `❌ AI Agent aip creation failed: ${error.message}
...
Error: ${error.toString()}`,
  ...
  examples: [
    [
      {
        name: 'User',
        content: { text: 'Create a aip plan with 20 tokens monthly for AI agent demo' },
      },
      {
        name: 'aip Manager AI',
        content: { 
          text: 'Creating aip plan for AI agent with real blockchain transactions!', 
          actions: ['CREATE_aip'] 
        },
      },
    ],
...
export const checkStatusAction: Action = {
  ...
  description: 'Shows current aip progress and balances for AI agent',
  ...
  handler: async (
    ...
    try {
      ...
      const [aipProgress, balances] = await Promise.all([
        web3Service.getaipProgress(aiAgentAddress).catch(() => null),
...
• 🏔️ Fuji (Home): ${balances.fuji} aip tokens
• 🌊 Echo (Yield): ${balances.echo} aip tokens  
• ⚡ Dispatch (Emergency): ${balances.dispatch} aip tokens
• **💎 Total Portfolio: ${balances.total} aip tokens**
...
**🎯 aip Progress:**
• Monthly Investment: ${aipProgress.monthlyAmount} aip tokens
• Progress: ${aipProgress.totalDeposited}/${aipProgress.totalTarget} tokens (${aipProgress.percentComplete}%)
• Goal: ${aipProgress.goal}
• Status: ${aipProgress.isActive ? '✅ Active' : '❌ Inactive'}
• Automation: ${aipProgress.automationEnabled ? '🤖 Gelato Active' : '⏸️ Paused'}
• Next Deposit: ${aipProgress.nextDepositTime.toLocaleDateString()}
...
• Fuji (Stable): ${balances.fuji} aip tokens
• Echo (Yield): ${balances.echo} aip tokens  
• Dispatch (Emergency): ${balances.dispatch} aip tokens
• Total Portfolio: ${balances.total} aip tokens

**🎯 Optimization Executed:**
• Moved ${optimizationAmount} aip from Fuji → Echo
...
function extractAmount(text: string): string | null {
  const amountMatch = text.match(/(\d+(?:\.\d+)?)\s*(?:tokens?|aip)/i);
