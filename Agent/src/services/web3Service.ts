...
export class Web3Service extends Service {
  ...
  capabilityDescription = 'Manages Web3 transactions and cross-chain aip operations using real smart contracts';
  ...
  private initializeChains() {
    this.chains = new Map([
      ['fuji', {
        ...
        contractAddress: process.env.FUJI_HOME_CONTRACT || '0x7CBC5bAd1aBbd269cC7b988422791F0586896cF3',
        name: 'Avalanche Fuji',
      ...
      ['echo', {
        ...
        contractAddress: process.env.ECHO_REMOTE_CONTRACT || '0x7CBC5bAd1aBbd269cC7b988422791F0586896cF3',
        name: 'Avalanche Echo',
      ...
      ['dispatch', {
        ...
        contractAddress: process.env.DISPATCH_REMOTE_CONTRACT || '0x4Ea92Ee7211aE6a9c5b5DDf10708509CE4c6c367',
  ...
  private initializeWallet() {
    ...
    const privateKey = process.env.AI_AGENT_PRIVATE_KEY || '1c5b654c94e0c649bdc86e0f11bc2de2ef36184ec172deaf555c851b0b09e650';
  ...
  private initializeContracts() {
    ...
    // Contract ABIs
    const homeABI = [
      "function createaip(uint256 monthlyAmount, uint256 totalTarget, uint256 intervalDays, string memory goal)",
      ...
      "function getaipProgress(address user) view returns (uint256, uint256, uint256, uint256, uint256, bool, bool, string)",
    ...
    this.aipToken = new ethers.Contract(
      process.env.aip_TOKEN_ADDRESS || '0x4Ea92Ee7211aE6a9c5b5DDf10708509CE4c6c367',
      tokenABI,
      fujiProvider
    );
  ...
  // ============================================
  // REAL aip FUNCTIONS WITH PROPER DEPOSITS
  // ============================================

  async createaip(userAddress: string, monthlyAmount: string, totalTarget: string, goal: string): Promise<TransactionResult> {
    try {
      // ALWAYS USE AI AGENT WALLET FOR EVERYTHING - NO USER METAMASK NEEDED
      const aiAgentAddress = this.aiWallet.address;
      logger.info(`🚀 Creating REAL aip for AI Agent ${aiAgentAddress}: ${monthlyAmount} monthly, target: ${totalTarget}`);
      ...
      // Step 2: Create the aip plan for AI agent (not user)
      const tx = await homeWithSigner.createaip(
        monthlyAmountWei,
        totalTargetWei,
      ...
      logger.info(`✅ aip Plan created for AI Agent! TX: ${receipt.hash}`);

      // Step 3: Execute IMMEDIATE first deposit using AI agent's own funds
      await this.executeImmediateDepositForAI(monthlyAmount);

      // Step 4: Start automated deposits via Gelato for AI agent
      setTimeout(() => this.scheduleGelatoDepositsForAI(monthlyAmount), 60000); // Start after 1 minute
      ...
      logger.error('❌ Failed to create aip:', error.message);
      throw error;
    }
  }
  ...
  async executeImmediateDepositForAI(amount: string): Promise<void> {
    try {
      const aiAgentAddress = this.aiWallet.address;
      logger.info(`🎯 EXECUTING IMMEDIATE DEPOSIT for AI Agent: ${amount} aip`);
      
      const homeWithSigner = this.homeContract.connect(this.aiWallet);
      const tokenWithSigner = this.aipToken.connect(this.aiWallet);
      const amountWei = ethers.parseEther(amount);
      
      // Check AI agent has enough tokens
      const aiBalance = await this.aipToken.balanceOf(aiAgentAddress);
      logger.info(`💰 AI Agent balance: ${ethers.formatEther(aiBalance)} aip tokens`);
      
      if (aiBalance < amountWei) {
        logger.info(`❌ Insufficient AI agent balance. Need ${amount} aip, have ${ethers.formatEther(aiBalance)}`);
        return;
      }
      
      // Approve the home contract to spend AI agent's tokens
      logger.info(`✅ Approving home contract to spend ${amount} aip tokens...`);
      ...
      // Execute the aip deposit for AI agent - this will trigger cross-chain transfers
      logger.info(`🚀 Triggering aip deposit for AI agent...`);
      const depositTx = await homeWithSigner.triggerUserDeposit(aiAgentAddress);
      const depositReceipt = await depositTx.wait();
      
      logger.info(`✅ IMMEDIATE aip DEPOSIT EXECUTED! TX: ${depositReceipt.hash}`);
  ...
  async scheduleGelatoDepositsForAI(monthlyAmount: string): Promise<void> {
    try {
      const aiAgentAddress = this.aiWallet.address;
      logger.info(`🤖 Starting Gelato automation for AI Agent: ${aiAgentAddress}...`);
      
      let depositCount = 0;
      const maxDeposits = 5; // Limit for demo
      
      // Simulate Gelato Automation by triggering deposits every 60 seconds
      const depositInterval = setInterval(async () => {
        try {
          ...
          logger.info(`🔄 Gelato Automation #${depositCount}: Triggering deposit for AI Agent`);
          ...
          logger.info(`✅ Gelato deposit #${depositCount} executed! TX: ${depositReceipt.hash}`);
          
          // Show updated balances after each deposit
          setTimeout(() => this.showBalanceDistribution(aiAgentAddress), 10000);
          
        } catch (error: any) {
          logger.error(`❌ Gelato deposit #${depositCount} failed:`, error.message);
        }
      }, 90000); // Every 90 seconds for demo visibility
      
    } catch (error: any) {
      logger.error('❌ Failed to schedule Gelato deposits:', error.message);
    }
  }
  ...
  async showBalanceDistribution(walletAddress: string): Promise<void> {
    try {
      ...
      logger.info(`   🏔️ Fuji: ${ethers.formatEther(fujiBalance)} aip`);
      logger.info(`   🌊 Echo: ${ethers.formatEther(echoBalance)} aip`);
      logger.info(`   ⚡ Dispatch: ${ethers.formatEther(dispatchBalance)} aip`);
      ...
      logger.info(`   💎 Total Distributed: ${total.toFixed(4)} aip tokens`);
      
      // Also check main token balance on Fuji
      const mainBalance = await this.aipToken.balanceOf(walletAddress);
      logger.info(`   🪙 Remaining on Fuji: ${ethers.formatEther(mainBalance)} aip tokens`);
  ...
  async crossChainTransfer(
    ...
    try {
      ...
      logger.info(`🌉 REAL cross-chain transfer: ${amount} aip from ${fromChain} to ${toChain}`);
      ...
      const sourceWallet = new ethers.Wallet(
        process.env.AI_AGENT_PRIVATE_KEY || '1c5b654c94e0c649bdc86e0f11bc2de2ef36184ec172deaf555c851b0b09e650', 
        provider
      );
      ...
      logger.info(`✅ AI agent has ${ethers.formatEther(sourceBalance)} aip on ${fromChain}`);
      ...
      logger.info(`📊 Transferred ${amount} aip from ${fromChain} to ${toChain}`);
  ...
  async getaipProgress(userAddress?: string): Promise<aipProgress> {
    try {
      // Always use AI agent address for real demo
      const targetAddress = userAddress || this.aiWallet.address;
      const result = await this.homeContract.getaipProgress(targetAddress);
      ...
      logger.error('Failed to get aip progress:', error.message);
      throw error;
    }
  }
